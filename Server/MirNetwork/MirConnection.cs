...
namespace Server.MirNetwork
{
    ...
    public class MirConnection
    {
        ...
        private void ReceiveData(IAsyncResult result)
        {
            ...
            // 作者: asm0x1 - 调试日志：记录收到的原始数据
            {
                byte[] rawBytesForLog = result.AsyncState as byte[];
                if (rawBytesForLog != null && dataRead > 0)
                {
                    string hexString = BitConverter.ToString(rawBytesForLog, 0, Math.Min(dataRead, 64));
                    MessageQueue.Enqueue($"[调试] 收到 {dataRead} 字节来自 {IPAddress}: {hexString}");
                }
            }
        ...
        public void Process()
        {
            ...
            {
                // 作者: asm0x1 - 调试日志：记录超时断开
                MessageQueue.Enqueue($"[调试] 连接超时断开(21) IP={IPAddress} Stage={Stage} Time={Envir.Time} TimeOut={TimeOutTime}");
                Disconnect(21);
                return;
            }
